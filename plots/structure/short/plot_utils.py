    ...
    for i,pcx in enumerate([0,2,4]):
        ...
        for cat in orderedCat:
            if cat != 'Real':
                ...
                win+=1
    # fig.suptitle(method)
    plt.savefig(outDir+"PCA_allel_compare_models_" + method + ".pdf", dpi=300, bbox_inches='tight')
...
def plotPCAdensity(pcdf, method, orderedCat, outDir):
    ...
    for pcs, cmap in [(('PC1', 'PC2'), "RdBu"), (('PC3', 'PC4'), "Reds_d"), (('PC5', 'PC6'), "Reds_d")]:
        g = sns.FacetGrid(pcdf, col="label", col_wrap=len(orderedCat), col_order=orderedCat)
        g.map(sns.kdeplot, pcs[0], pcs[1], cmap=cmap, cbar=True)
        g.set_titles("{col_name}")  # This removes "label = " from titles
        pdf.savefig(dpi=300, bbox_inches='tight')
...
def plotPrivacyLoss(Train, Test, outDir, colpal, allcolpal):
    ...
    if not os.path.exists(outDir+f'AA{Train}.csv.bz2') or not os.path.exists(outDir+f'AA{Test}.csv.bz2'): 
        ...
        plt.ylim(-.5,.5)
        plt.savefig(outDir + f"PrivacyLoss_Train{Train}_Test{Test}.pdf", dpi=300, bbox_inches='tight')
