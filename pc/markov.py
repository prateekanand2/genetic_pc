...
def train_model(train_subset, valid_subset, chunk_id):
    ...
    tag = f"chunk_{chunk_id}"
    log_filename = f"{CFG.output_prefix}_{tag}_ps{CFG.ps}.log"
    ...
    with open(log_filename, "w") as log_file:
        for epoch in range(1, CFG.num_epochs + 1):
            ...
            # 1. Training Pass (Accumulate Flows)
            ...
            # 3. Calculate Final Train LL (Post-EM)
            train_ll = 0.0
            with torch.no_grad():
                for batch in train_loader:
                    x = batch[0].to(CFG.device)
                    train_ll += pc(x).mean().item()
            ...
            stats = (f"[Epoch {epoch}/{CFG.num_epochs}] Train LL: {train_ll:.4f}; "
                     f"Val LL: {valid_ll:.4f} | Train Time: {t1-t0:.2f}s | Val Time: {t2-t1:.2f}s")
            print(stats)
